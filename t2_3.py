...
def generate_pseudo_labels(train_dataset, unlabeled_indices, device, k=5):
    ...
    pretrained_model = resnet18(pretrained=True)  # 使用旧版参数
    pretrained_model.fc = nn.Identity()
    pretrained_model = pretrained_model.to(device)
    pretrained_model.eval()
    
    # Extract features from all samples
    features, labels = extract_features(train_dataset, pretrained_model, device)
...
if __name__ == "__main__":
    ...
    arg = f"pretrain_knn_k_{knn_k}_lr_{lr}_dropout_{dropout}_bz_{BATCH_SIZE}"
