...
def generate_pseudo_labels_with_model(train_dataset, unlabeled_indices, device, threshold=0.9):
    # Extract labeled data
    labeled_indices = [i for i in range(len(train_dataset)) if i not in unlabeled_indices]
    labeled_paths = [train_dataset.image_paths[i] for i in labeled_indices]
    ...
    train_transform = transforms.Compose([
        ...
        transforms.RandomRotation(20), transforms.ColorJitter(0.1,0.1,0.1,0.1),
        transforms.ToTensor(), transforms.Normalize([0.485,0.456,0.406],[0.229,0.224,0.225])])
    
    # Train initial model
    model = MalariaCNN().to(device)
    criterion = nn.CrossEntropyLoss(); optimizer = optim.Adam(model.parameters(), lr=0.001)
    labeled_dataset = MalariaDataset(labeled_paths, labeled_labels, train_transform)
    labeled_loader = DataLoader(labeled_dataset, BATCH_SIZE, shuffle=True, num_workers=8, drop_last=True)
    ...
    val_transform = transforms.Compose([
        ...
        transforms.Normalize([0.485,0.456,0.406],[0.229,0.224,0.225])])
    unlabeled_paths = [train_dataset.image_paths[i] for i in unlabeled_indices]
    unlabeled_dataset = MalariaDataset(unlabeled_paths, [0]*len(unlabeled_paths), val_transform)
    unlabeled_loader = DataLoader(unlabeled_dataset, BATCH_SIZE, shuffle=False, num_workers=8)
    ...
    with torch.no_grad():
        for images, _ in unlabeled_loader:
            ...
            outputs = model(images); prob = torch.softmax(outputs, dim=1)
            conf, pred = torch.max(prob, dim=1)
            pseudo_labels.extend(pred.cpu().numpy().tolist())
            confidences.extend(conf.cpu().numpy().tolist())
    
    # Select high-confidence samples
    selected_paths = [p for p,c in zip(unlabeled_paths, confidences) if c >= threshold]
    selected_labels = [l for l,c in zip(pseudo_labels, confidences) if c >= threshold]
    
    return labeled_paths + selected_paths, labeled_labels + selected_labels

def prepare_datasets(random_sample_ratio=0.0, transform=True, threshold=0.9):
    ...
    # Split datasets
    train_paths, test_paths, train_labels, test_labels = train_test_split(all_paths, labels, test_size=0.2, random_state=0)
    train_paths, val_paths, train_labels, val_labels = train_test_split(train_paths, train_labels, test_size=0.1, random_state=0)
    ...
    if random_sample_ratio > 0:
        ...
        selected_train_paths, selected_train_labels = generate_pseudo_labels_with_model(
            initial_train_dataset, unlabeled_indices, device, threshold)
    else:
        selected_train_paths, selected_train_labels = train_paths, train_labels
    ...
    return (
        MalariaDataset(selected_train_paths, selected_train_labels, train_transform),
        MalariaDataset(val_paths, val_labels, val_test_transform),
        MalariaDataset(test_paths, test_labels, val_test_transform),
        random_sample_ratio
    )
...
if __name__ == "__main__":
    ...
    arg = f"st_lr_{lr}_dropout_{dropout}_bz_{BATCH_SIZE}"
    ...
    # Prepare data
    full_train_dataset, full_val_dataset, full_test_dataset, _ = prepare_datasets(random_sample_ratio=0.0)
    ...
    for random_sample_ratio in ratios:
        ...
        train_dataset, val_dataset, test_dataset, _ = prepare_datasets(random_sample_ratio)
