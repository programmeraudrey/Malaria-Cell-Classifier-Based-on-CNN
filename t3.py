...
class SymmetricCrossEntropy(nn.Module):
    ...
    def forward(self, logits, targets):
        ...
        rce_loss = -torch.sum(pred_probs * torch.log(torch.clamp(onehot_targets, min=1e-8)), dim=1)
        loss = self.beta * ce_loss + self.alpha * rce_loss
        return loss.mean()
...
class GeneralizedCrossEntropy(nn.Module):
    ...
    def forward(self, logits, targets):
        pred_probs = F.softmax(logits, dim=1)
        correct_probs = pred_probs.gather(1, targets.view(-1,1)).squeeze()
        loss = (1.0 - torch.pow(correct_probs, self.q)) / self.q
        return loss.mean()

class MAELoss(nn.Module):
    def forward(self, logits, targets):
        pred_probs = F.softmax(logits, dim=1)
        targets_onehot = F.one_hot(targets, num_classes=pred_probs.size(1)).float().to(logits.device)
        return torch.mean(torch.abs(pred_probs - targets_onehot))
...
class BootstrapCE(nn.Module):
    ...
    def forward(self, logits, targets):
        pred_probs = F.softmax(logits, dim=1)
        targets_onehot = F.one_hot(targets, num_classes=pred_probs.size(1)).float().to(logits.device)
        new_targets = self.beta * targets_onehot + (1 - self.beta) * pred_probs.detach()
        return -torch.sum(new_targets * F.log_softmax(logits, dim=1), dim=1).mean()
...
def prepare_datasets(random_sample_ratio=1.0, transform=True):
    ...
    if random_sample_ratio > 0:
        ...
        num_flip = int(num_samples * random_sample_ratio)
        flip_indices = random.sample(range(num_samples), num_flip)
        for idx in flip_indices:
            train_labels[idx] = 1 - train_labels[idx]  # 翻转标签
...
# Training function
def train_model(model, train_loader, val_loader, device, ckpt_path, lr=0.001, loss_type = "ce"):
    # 主函数中的训练循环
    loss_functions = {
        'ce': nn.CrossEntropyLoss(),
        'sce': SymmetricCrossEntropy(alpha=0.1, beta=1.0),
        'gce': GeneralizedCrossEntropy(q=0.7),
        'mae': MAELoss(),
        'bootstrap': BootstrapCE(beta=0.8)
    }
...
# Test function
def test_model(model, test_loader, device, best_epoch, loss_type = "ce"):
    model.eval()
    correct = 0
    total = 0
    test_loss = 0.0
...
if __name__ == "__main__":
    ...
    loss_type = 'bootstrap'
    arg = f"loss_type_{loss_type}_lr_{lr}_dropout_{dropout}_bz_{BATCH_SIZE}"
    ...
    # 重定向标准输出到日志文件
    log_path = f'./logs/t2_training_log_{arg}_{filename_time}.txt'
    ...
    full_ckpt_path = f'./checkpoints/t2_best_malaria_cnn_{full_args}_{filename_time}.pth'
    ...
    full_train_loss, full_train_acc, full_val_loss, full_val_acc, full_best_epoch = train_model(
        full_model, full_train_loader, full_val_loader, device, full_ckpt_path, lr, loss_type
    )
    ...
    baseline_accuracy = test_model(full_model, full_test_loader, device, full_best_epoch, loss_type)
    ...
    for random_sample_ratio in ratios:
        ...
        ckpt_path = f'./checkpoints/t2_best_malaria_cnn_{args}_{filename_time}.pth'
        ...
        train_loss, train_acc, val_loss, val_acc, best_epoch = train_model(
            model, train_loader, val_loader, device, ckpt_path, lr, loss_type
        )
        ...
        test_acc = test_model(model, test_loader, device, best_epoch, loss_type)
        ...
        plt.tight_layout()
        plt.savefig(f"./pngs/t2_training_curv_{args}_{filename_time}.png")
        plt.close()
